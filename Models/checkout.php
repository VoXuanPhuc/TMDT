...
class Checkout extends model
{
	function save($data)
	{
		...
		foreach ($_SESSION['products'] as $value) {
			...
			$getSlspQuery = "SELECT SoLuong from sanpham where MaSP = $MaSP";

			$currentSLSanPham = $this->conn->query($getSlspQuery)->fetch_assoc();
			$countsl = (int) $currentSLSanPham['SoLuong'] - (int) $SoLuong;

			$updateSlspQuery = "UPDATE sanpham set SoLuong = $countsl where MaSP = $MaSP";

			$currentSLSanPham = $this->conn->query($updateSlspQuery);
